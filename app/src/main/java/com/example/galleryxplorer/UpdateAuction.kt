...
class UpdateAuction : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        itemName = findViewById(R.id.et_update_auction_name)
        itemCategory = findViewById(R.id.et_update_auction_category)
        itemMedium = findViewById(R.id.et_update_auction_medium)
        itemSubject = findViewById(R.id.et_update_auction_subject)
        itemYear = findViewById(R.id.et_update_auction_year)
        itemSize = findViewById(R.id.et_update_auction_size)
        itemPrice = findViewById(R.id.et_update_auction_price)
        sellerId = findViewById(R.id.et_update_auction_sellerId)
        randomId = findViewById(R.id.et_update_auction_randomId)
        itemMainImage = findViewById(R.id.auction_update_image1)
        itemSecondImage = findViewById(R.id.auction_update_image2)
        itemThirdImage = findViewById(R.id.auction_update_image3)

        btnUpdate = findViewById(R.id.btn_update_auction)
    ...
    private fun updateAuction() {
        val auctionNameText = itemName.text.toString()
        val auctionCategoryText = itemCategory.text.toString()
        val auctionMediumText = itemMedium.text.toString()
        val auctionSubjectText = itemSubject.text.toString()
        val auctionYearText = itemYear.text.toString()
        val auctionSizeText = itemSize.text.toString()
        val auctionPriceText = itemPrice.text.toString()
        ...
        if (auctionNameText.isEmpty() || auctionCategoryText.isEmpty() || auctionMediumText.isEmpty() || auctionSubjectText.isEmpty() ||
            auctionYearText.isEmpty() || auctionSizeText.isEmpty() || auctionPriceText.isEmpty()
        ) {
                ...
                uploadTask.addOnSuccessListener { taskSnapshot ->
                    taskSnapshot.metadata!!.reference!!.downloadUrl.addOnSuccessListener { uri ->
                        ...
                        if(updatedImageUrls.size >=1 ){
                            val updatedAuctionData = mapOf(
                                "itemName" to auctionNameText,
                                "itemCategory" to auctionCategoryText,
                                "itemMedium" to auctionMediumText,
                                "itemSubject" to auctionSubjectText,
                                "itemYear" to auctionYearText,
                                "itemSize" to auctionSizeText,
                                "itemPrice" to auctionPriceText,
                                "urls" to updatedImageUrls
                            )

                            database.collection("allAuctions").document(intent.getStringExtra("randomId").toString())
                                .update(updatedAuctionData)
                            database.collection("auctioncategory").document("$auctionCategoryText")
                                .collection("categoryAuctions").document(intent.getStringExtra("randomId").toString())
                                .set(updatedAuctionData)
                            database.collection("sellerAuctionsBySellerID")
                                .document(intent.getStringExtra("sellerId").toString())
                                .collection("sellerAuctions")
                                .document(intent.getStringExtra("randomId").toString()).update(updatedAuctionData)
