                        ...
                        uploadTask.addOnSuccessListener { taskSnapshot ->
                            taskSnapshot.metadata!!.reference!!.downloadUrl.addOnSuccessListener { uri ->
                                ...
                                if (uploadedImageUrls.size >= 1) {
                                    ...
                                    database.collection("allClasses").document(randomId).set(sellerMap)
                                    database.collection("classcategory").document("$iCategory").collection("categoryClasses").document(randomId).set(sellerMap)
                                    database.collection("sellerClassesBySellerID").document(uId)
                                        .collection("sellerClasses").document(randomId)
                                        .set(sellerMap)
