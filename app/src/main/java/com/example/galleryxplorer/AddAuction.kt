...
class AddAuction : AppCompatActivity() {
                        ...
                        uploadTask.addOnSuccessListener { taskSnapshot ->
                            taskSnapshot.metadata!!.reference!!.downloadUrl.addOnSuccessListener { uri ->
                                ...
                                if (uploadedImageUrls.size >= 1) {
                                    ...
                                    database.collection("allAuctions").document(randomId).set(sellerMap)
                                    database.collection("auctioncategory").document("$iCategory").collection("categoryAuctions").document(randomId).set(sellerMap)
                                    database.collection("sellerAuctionsBySellerID").document(uId)
                                        .collection("sellerAuctions").document(randomId)
                                        .set(sellerMap)
